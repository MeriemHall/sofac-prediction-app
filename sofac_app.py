...
def main():
    ...
    with st.sidebar:
        ...
        # Create the performance metrics HTML safely
        r2_value = st.session_state.r2
        ...
        st.sidebar.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
        st.info("↻ Surveillance économique continue active")
        st.sidebar.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        st.markdown(complete_panel, unsafe_allow_html=True)
...
def display_live_data_panel(live_data, last_historical_yield):
    ...
    """
    st.sidebar.markdown(update_html, unsafe_allow_html=True)
